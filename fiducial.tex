...
\noindent\textbf{Contributions:}~~To address these problems, we design a
...
\begin{figure}[t!]
  ...
  \caption[Overview]{Experimental setup and comparison of fiducial-based
    ...
    \textbf{(Right, Bottom)} Output of our fiducial. Green
...
We designed a binary coded fiducial that uses concentric white rings of
...
(Section~\ref{sec:blurtest}) on our fiducial. Specifically,
...
rate of fiducials.  Our fiducial handsomely outperforms prior methods.}
...
\begin{table}[t!]
  ...
  \label{tab:recognition_accuracy} Recognition rate of ARTag and our fiducials
  on real data captured through AR Drone. Each row shows analysis of a test
...
similar conditions, our fiducials are successfully recognized. The overall recognition rate of proposed
fiducials is 90\% while that of ARTags is around 60\%. 
\textbf{Bottom Rows} Time view of recognition. ARTag is ``choppy'' with
frequent losses, while our fiducial (extreme bottom) is ``up'' almost all the 
time.}
\label{fig:setup}
\end{figure*}
...
are able to contrast the performance of BLUT~\cite{Wu:2011} with our method. 
From the images in the top rows of Figure~\ref{fig:BLUT_compare_00}--\ref{fig:BLUT_compare_11},
...
the recognition rate of existing fiducials. We designed a blur
resilient fiducial. Our design of contrasting concentric rings is based on the
observation that the direction perpendicular to the motion blur direction will be
