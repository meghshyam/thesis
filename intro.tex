...
\end{figure}
  ...
  etc. as the detection rate of ARTag is better than other existing fiducials.
  However, none of these fiducials are detected in the presence of motion blur. Figure
  \ref{fig:ARTagBlur} demonstrates the problem in detection of ARTag under
  motion blur. Figure \ref{fig:ARTagBlur} (Left) shows that ARTags get detected
  (indicated by green rectangles) in the absence of blur in captured images.
  However, when there is a significant amount of blur, ARTags cannot be
  detected (Figure \ref{fig:ARTagBlur} (Right)).
